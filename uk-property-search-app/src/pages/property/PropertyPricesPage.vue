  ...
  .row.q-gutter-x-lg.items-end
    ...
    .col
      q-range(v-model='priceRange' :min='minPrice' :max='maxPrice' :step='step' label-always markers
             :left-label-value='formatSliderLabel(priceRange.min)' :right-label-value='formatSliderLabel(priceRange.max)')
    .col-shrink
      q-checkbox(v-model='includeBeyondPriceRange' :label='`Include ${formatSliderLabel(maxPrice)}+`')
